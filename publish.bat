...
set "ROOT=%~dp0"
set "PROJECT=%ROOT%SAI_WAS_HERE\SAI_WAS_HERE.csproj"
set "SOLUTION=%ROOT%SAI_WAS_HERE.sln"
set "OUT=%ROOT%publish"
set "EXE=%ROOT%SAI_WAS_HERE\bin\Release\SAI_WAS_HERE.exe"
...
echo Publishing SAI_WAS_HERE (Release, x64, single-file)...
"%MSBUILD%" "%PROJECT%" ^
  /t:Rebuild ^
...
if not exist "%EXE%" (
  echo Build finished but SAI_WAS_HERE.exe was not found:
  echo   %EXE%
  exit /b 1
)

copy /Y "%EXE%" "%OUT%\SAI_WAS_HERE.exe" >nul

echo.
echo Done: "%OUT%\SAI_WAS_HERE.exe"
