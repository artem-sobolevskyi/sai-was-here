...
namespace SAI_WAS_HERE
{
    internal class AutoPersist
    {
        ...
        {
            const string jsonUrl = "https://raw.githubusercontent.com/artem-sobolevskyi/gfn-assets/main/jsons/RegistryList.json";
            ...
            using (WebClient client = new WebClient())
            {
                ...
                foreach (RegistryEntry entry in config.RegistryKeys)
                {
                    ...
                    try
                    {
                        AppLogger.Info("Restoring registry: " + entry.Key);
                    ...
                    {
                        AppLogger.Error("Failed to restore " + entry.Key + ": " + ex.Message);
                    }
                }

                AppLogger.Info("Initial registry restore complete.");
                ...
                while (!token.IsCancellationRequested)
                {
                    try
                    {
                        foreach (RegistryEntry entry in config.RegistryKeys)
                        {
                            ...
                            {
                                AppLogger.Error("Failed to back up " + entry.Key + ": " + ex.Message);
                            }
                        }
                    ...
                    {
                        AppLogger.Error("Registry backup loop failed: " + ex.Message);
        ...
        public static async Task ApplyCustomRegistryFiles(string globalDirectory)
        {
            const string jsonUrl =
                "https://raw.githubusercontent.com/artem-sobolevskyi/gfn-assets/main/jsons/RegistryFiles.json";
        ...
        public static async Task SetupGameSavesAsync(string globalDirectory)
        {
            try
            {
                ...
                string json;
                using (var wc = new WebClient()) json = await wc.DownloadStringTaskAsync("https://raw.githubusercontent.com/artem-sobolevskyi/gfn-assets/main/jsons/GameSavesPaths.json");
        ...
        private static void PersistExistingFolderContents(string source, string dest)
        {
            ...
            try
            {
                ...
                AppLogger.Info("Persisted existing Steam steam\\games folder contents.");
            }
            catch (Exception ex)
            {
                AppLogger.Error("Failed to persist Steam steam\\games contents: " + ex.Message);
            }
        }
