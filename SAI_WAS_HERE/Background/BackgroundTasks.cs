...
namespace SAI_WAS_HERE
{
    internal static class BackgroundTasks
    {
        ...
        public static async Task StartEacWatcherAsync(CancellationToken token)
        {
            ...
            try
            {
                while (!token.IsCancellationRequested)
                {
                    ...
                    foreach (var processName in eacProcessNames)
                    {
                        ...
                        foreach (var proc in runningProcs)
                        {
                            ...
                            { 
                                ...
                                {
                                    proc.Kill(); 
                                    AppLogger.Warn($"Terminated blocked process: {proc.ProcessName}");
                                    eacTerminated = true;
                                }
                            } 
                            catch { }
                    ...
                    if (eacTerminated)
                    {
                        _ = Task.Run(() => MessageBox.Show("Easy Anti-Cheat processes have been terminated to prevent session issues. Anti-Cheat games don't work.", "SAI_WAS_HERE", MessageBoxButtons.OK, MessageBoxIcon.Information));
        ...
        // Monitors Desktop and Start Menu shortcuts, syncing them to the persistent SAI_WAS_HERE directory
            ...
            if (!File.Exists(targetDesktopPath))
            {
                ...
                if (File.Exists(sourcePath))
                {
                    ...
                    { 
                        ...
                        new Thread(() => MessageBox.Show($"{Path.GetFileNameWithoutExtension(name)} is a core component and cannot be removed.", "SAI_WAS_HERE", MessageBoxButtons.OK, MessageBoxIcon.Information)).Start();
                    } 
                    catch { }
                }
            }
            ...
            try
            {
                while (!token.IsCancellationRequested)
                {
                    ...
                    if (windowPtr != IntPtr.Zero)
                    {
                        ...
                        AppLogger.Info("CustomExplorer has been closed.");
                    }
                }
            }
            catch (TaskCanceledException) { }
        ...
        public static Task StartBrickPreventionAsync(CancellationToken token)
        {
            ...
            {
                AppLogger.Error($"Brick prevention task failed: {ex.Message}, MAKE SURE YOU DO NOT USE THE STEAM LAUNCH OPTIONS");
                return Task.CompletedTask;
            }
        }
                ...
                for (int i = 0; i < 3; i++)
                {
                    try
                    {
                        ...
                        if (content.Contains(blackListed))
                        {
                            ...
                            AppLogger.Error("STEAM LAUNCH OPTIONS DETECTED. Session terminated.");
        ...
        public static void EnvironmentSetup()
        {
            try
            {
                AppLogger.Info("Setting up environment variables...");
                ...
                string powershellRoot = @"I:\Apps\SAI_WAS_HERE\SilentApps\PowerShell";

                using (RegistryKey key = Registry.CurrentUser.CreateSubKey("Environment"))
                {
                    if (key == null)
                    {
                        AppLogger.Error("Could not open HKCU\\Environment.");
                        return;
                    }
                ...
                AppLogger.Info("Registry updated.");
                ...
                AppLogger.Info("Current process environment updated.");
                ...
                AppLogger.Info("Environment change broadcast sent.");

                using (RegistryKey verify = Registry.CurrentUser.OpenSubKey("Environment"))
                {
                    AppLogger.Info("Verification:");
                    AppLogger.Info("DOTNET_ROOT = " + verify.GetValue("DOTNET_ROOT", ""));
                    AppLogger.Info("POWERSHELL_ROOT = " + verify.GetValue("POWERSHELL_ROOT", ""));
                }

                AppLogger.Info("Environment setup complete.");
            }
            catch (Exception ex)
            {
                AppLogger.Error("Environment setup failed: " + ex.Message);
