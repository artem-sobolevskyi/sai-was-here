...
namespace SAI_WAS_HERE
{
    internal static class FinalBackgroundTasks
    {
        public static async Task OpenShellStartup(string globalDirectory)
        {
            ...
            {
                AppLogger.Error("Desktop finalization failed: " + ex.Message);
            }
        }
        ...
        private static async Task CloseStartHookWindowAsync()
        {
            ...
            while (DateTime.UtcNow < deadline)
            {
                try
                {
                    ...
                    if (window != IntPtr.Zero)
                    {
                        ...
                        AppLogger.Info("StartHookWindow has been closed.");
                        return;
                    }
                }
                catch { }
            ...
            AppLogger.Warn("StartHookWindow not found.");
        }
        ...
        private static void RemoveDxCache(string globalDirectory)
        {
            ...
            try
            {
                ...
                AppLogger.Info("Removed Game Saves\\DXCache.");
            }
            catch (Exception ex)
            {
                AppLogger.Error("Failed to remove Game Saves\\DXCache: " + ex.Message);
            }
        }
        ...
        private static void ApplyFileAssociations(string globalDirectory)
        {
            ...
            {
                AppLogger.Warn("SetUserFTA.exe not found. Skipping file associations.");
                return;
            }
            ...
            AppLogger.Info("File associations applied.");
        }
        ...
        private static void RegisterOpenCommand(string keyPath, string exePath)
        {
            ...
            {
                AppLogger.Warn("Skipping file association command, missing: " + exePath);
                return;
            }
        ...
        private static void SetUserFta(string setUserFta, string extension, string progId)
        {
            ...
            {
                AppLogger.Error("SetUserFTA " + extension + " failed: " + ex.Message);
            }
        }
        ...
        private static void ApplyDesktopContextMenus(string globalDirectory)
        {
            AddBackgroundMenuCommand(
                ...
                Path.Combine(globalDirectory, "SilentApps", "ChangeWallpaper.exe"));
            ...
            AppLogger.Info("Desktop context menus applied.");
        }
        ...
        private static void AddBackgroundMenuCommand(string keyName, string menuName, string exePath)
        {
            if (!File.Exists(exePath))
            {
                AppLogger.Warn("Skipping context menu " + menuName + ", missing: " + exePath);
                return;
            }
        ...
        private static void CreateSteamDesktopShortcut(string globalDirectory)
        {
            ...
            {
                AppLogger.Info("Steam shortcut already saved. Skipping desktop create.");
                return;
            }
            ...
            AppLogger.Info("Steam desktop shortcut created.");
        }
        ...
        private static void PinOpenShellShortcuts()
        {
            ...
            foreach (string shortcut in shortcuts)
            {
                ...
                try
                {
                    ...
                    AppLogger.Info("Pinned Open-Shell shortcut: " + shortcut);
                }
                catch (Exception ex)
                {
                    AppLogger.Error("Failed to pin " + shortcut + ": " + ex.Message);
                }
            }
        }
        ...
        private static void ResetExplorerPlusPlus()
        {
            ...
            try
            {
                ...
                AppLogger.Info("Removed Explorer++ registry key.");
            }
            catch (Exception ex)
            {
                AppLogger.Error("Failed to remove Explorer++ registry: " + ex.Message);
            }
        }
        ...
        private static void StartOpenShell(string globalDirectory)
        {
            ...
            {
                AppLogger.Warn("Open-Shell StartMenu.exe not found.");
                return;
            }
            ...
            AppLogger.Info("Open-Shell started.");
        }

        private static void ShowFirstRunNotice(string globalDirectory)
        {
            string marker = Path.Combine(globalDirectory, "SAI_WAS_HEREWelcome.shown");
            ...
            var thread = new Thread(() =>
            {
                try
                {
                    ...
                    AppLogger.Info("First-run notice acknowledged.");
                }
                catch (Exception ex)
                {
                    AppLogger.Error("First-run notice failed: " + ex.Message);
                }
            });
        ...
        private const string WelcomeTitle = "SAI_WAS_HERE";
