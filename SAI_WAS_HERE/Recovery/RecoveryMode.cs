...
namespace SAI_WAS_HERE
{
    internal class RecoveryMode
    {
        ...
        public static async Task ShowRecoveryPrompt()
        {
            ...
            {
                new MenuOption(
                    "Factory reset (This will remove the entire SAI_WAS_HERE folder and perform a fresh installation. Everything else on the I: drive will remain untouched.)",
                    FactoryReset),
        ...
        static async Task FactoryReset()
        {
            ...
            {
                AppLogger.Error($"Global directory '{globalDirectory}' does not exist, make sure you first install SAI_WAS_HERE.");
        ...
        static async Task RestoreShortcuts()
        {
            ...
            const string jsonUrl = "https://raw.githubusercontent.com/artem-sobolevskyi/gfn-assets/main/jsons/apps.json";
            ...
            try
            {
                ...
                using (var wc = new WebClient())
                {
                    var dir = JsonConvert.DeserializeObject<List<SavePath>>(
                        await wc.DownloadStringTaskAsync("https://raw.githubusercontent.com/artem-sobolevskyi/gfn-assets/main/jsons/directory.json"))[0];
                    ...
                    var apps = JsonConvert.DeserializeObject<List<Apps>>(json)
                        .Where(app => !string.Equals(app.name, "Roblox Cursor Fix", StringComparison.OrdinalIgnoreCase));
        ...
        static async Task InstallExplorer()
        {
            ...
            try
            {
                ...
                string explorerUrl = await AssetRemotes.GetUrlAsync("explorerPlusPlus");
                using (var wc = new WebClient())
                {
                    await wc.DownloadFileTaskAsync(new Uri(explorerUrl), destination);
                }

                Process.Start(destination);

                AppLogger.Info("Explorer++ downloaded successfully.");
            }
            catch (Exception ex)
            {
                AppLogger.Error(ex.ToString());
        ...
        static async Task InstallSystemInformer()
        {
            ...
            {
                string exeDirectory = Path.GetDirectoryName(Process.GetCurrentProcess().MainModule.FileName);
                string systemInformerDirectory = Path.Combine(exeDirectory, "System Informer");
                string systemInformerExePath = Path.Combine(systemInformerDirectory, "SystemInformer.exe");

                string zipPath = Path.Combine(exeDirectory, "System.Informer.zip");
                ...
                string informerUrl = await AssetRemotes.GetUrlAsync("systemInformerZip");
                using (var wc = new WebClient())
                {
                    await wc.DownloadFileTaskAsync(new Uri(informerUrl), zipPath);
                }
