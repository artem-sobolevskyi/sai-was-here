...
namespace SAI_WAS_HERE
{
    internal static class DesktopInstaller
    {
        ...
        public static async Task DesktopInstallAsync(string globalDirectory)
        {
            ...
            const string jsonUrl = "https://raw.githubusercontent.com/artem-sobolevskyi/gfn-assets/main/jsons/ExplorerDesktop.json";
            ...
            catch (Exception ex) { AppLogger.Error("Failed to set Dark Mode: " + ex.Message); }

            // 2. Set wallpaper: user file in DesktopWallpaper, otherwise download default from GitHub
            Directory.CreateDirectory(userWallpaperDir);
            Directory.CreateDirectory(defaultWallpaperDir);

            string defaultWallpaperPath = Path.Combine(defaultWallpaperDir, "wallpaper.jpeg");
            const string wallpaperUrl = "https://raw.githubusercontent.com/artem-sobolevskyi/gfn-assets/main/wallpaper.jpeg";
            ...
            try
            {
                using (var webClient = new WebClient())
                {
                    ...
                    webClient.Headers.Add("User-Agent", "SAI_WAS_HERE");
                    await webClient.DownloadFileTaskAsync(new Uri(wallpaperUrl), defaultWallpaperPath);
                }
            }
            catch (Exception ex)
            {
                AppLogger.Error("Failed to download default wallpaper: " + ex.Message);
            }
            ...
            string wallpaperToApply = wallpaper ?? (File.Exists(defaultWallpaperPath) ? defaultWallpaperPath : null);
            ...
            try
            {
                ...
                foreach (var desktop in desktopInfo)
                {
                    ...
                    if (string.Equals(desktop.run, "true", StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        AppLogger.Info("Starting desktop app: " + exePath);
                ...
                if (AppSettings.BingWallpaperEnabled)
                {
