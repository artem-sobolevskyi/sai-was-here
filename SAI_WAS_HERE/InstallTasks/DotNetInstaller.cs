...
namespace SAI_WAS_HERE
{
    internal static class DotNetInstaller
    {
        ...
        {
            AppLogger.Info("Starting .NET runtime install...");
            ...
            try
            {
                ...
                if (pending.Length == 0)
                {
                    AppLogger.Info("All required .NET runtimes are already installed.");
                    return;
                }
                ...
                var packs = (await Task.WhenAll(pending.Select(async channel =>
                {
                    ...
                    {
                        AppLogger.Error($"Could not resolve latest .NET {channel} version.");
                ...
                AppLogger.Info("Downloading all .NET zips...");
                ...
                if (broken.Length > 0)
                {
                    AppLogger.Error("Not all .NET zips are intact. Skipping extract: " + string.Join(", ", broken));
                    CleanupZips(packs);
                    return;
                }

                AppLogger.Info("All .NET zips downloaded and intact. Extracting to " + installDir);
                ...
                foreach (RuntimePack pack in packs)
                {
                    ...
                    try
                    {
                        ...
                        AppLogger.Info("Extracted " + pack.Label);
                    }
                    catch (Exception ex)
                    {
                        AppLogger.Error("Failed to extract " + pack.Label + ": " + ex.Message);
                    }
                }
                ...
                if (failed.Length == 0)
                    AppLogger.Info("Successfully installed all .NET runtimes.");
                else
                    AppLogger.Error("DotNet install finished with missing runtimes: " + string.Join(", ", failed));
            }
            catch (OperationCanceledException) { }
            catch (Exception ex)
            {
                AppLogger.Error("DotNet install failed: " + ex.Message);
            }
        }
        ...
        private static async Task DownloadPackAsync(RuntimePack pack, CancellationToken token)
        {
            try
            {
                AppLogger.Info("Downloading " + pack.Label + "...");
                pack.ZipPath = await DownloadFileAsync(pack.Urls, token);
            }
            catch (Exception ex)
            {
                AppLogger.Error("Download failed for " + pack.Label + ": " + ex.Message);
            }
        }

        private static async Task<string> DownloadFileAsync(List<string> urls, CancellationToken token)
        {
            string zipPath = Path.Combine(Path.GetTempPath(), "sai_was_here-dotnet-" + Guid.NewGuid().ToString("N") + ".zip");
        ...
        private static void ExtractZipEntries(string zipPath, string destDir)
        {
            ...
            using (var archive = ZipFile.OpenRead(zipPath))
            {
                foreach (ZipArchiveEntry entry in archive.Entries)
                {
                    ...
                    {
                        AppLogger.Warn("Skipped zip entry " + entry.FullName + ": " + ex.Message);
                    }
                }
            }
        }
