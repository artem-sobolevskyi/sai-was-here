...
namespace SAI_WAS_HERE
{
    internal static class AppInstaller
    {
        ...
        {
            const string jsonUrl = "https://raw.githubusercontent.com/artem-sobolevskyi/gfn-assets/main/jsons/appsV2.json";
            try
            {
                List<Apps> apps;
                ...
                var tasks = apps.Select(app => Task.Run(async () =>
                {
                    using (var webClient = new WebClient())
                    {
                        ...
                        if (isExe)
                        {
                            AppLogger.Info("Downloading " + app.name);
                        ...
                        {
                            if (alreadyExists && hasNewerVersion)
                                AppLogger.Info($"Updating {app.name} to {versionKey}");
                            else
        ...
        {
            const string jsonUrl = "https://raw.githubusercontent.com/artem-sobolevskyi/gfn-assets/main/jsons/silentappsV2.json";
            ...
            try
            {
                ...
                var tasks = apps.Select(app => Task.Run(async () =>
                {
                    using (var webClient = new WebClient())
                    {
                        ...
                        if (alreadyExists && hasNewerVersion)
                            AppLogger.Info($"Updating silent app {app.name} to {versionKey}");
                        else if (!alreadyExists)
                            AppLogger.Info("Installing silent app " + app.name);
        ...
        private static void WriteVersionMarker(string versionMarkerFile, string remoteFileName)
        {
            ...
            catch (Exception ex) { AppLogger.Error($"Failed to write version marker: {ex.Message}"); }
        }
